...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('otterDr.openWebview', async () => {
      ...
      // SOMEWHERE AROUND HERE - create function/method to push ai translations to webview, variables for whatHappened, nextSteps, otterThoughts
...
class OtterViewProvider implements vscode.WebviewViewProvider {
  ...
  public sendErrorsToWebview(numOfErrors: number) {
    if (this._view) {
      this._view.webview.postMessage({
        type: 'SET_ERRORS',
        errors: numOfErrors, // number of errors in current diagnostic array
      });
    }
  }
  private _getHtmlForWebview(webview: vscode.Webview) {
    ...
    return `<!DOCTYPE html>
     ...
     <body>
       ...
       console.log('Error count in webview: ', message.numOfErrors);
       }
       });
       <script>
     </body>
